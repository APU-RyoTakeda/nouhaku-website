  ...
  return (
    <footer className="bg-gray-900 text-white py-12 px-4">
      <div className="container mx-auto flex flex-col items-center">
        {/* ロゴ部分 */}
        <div className="mb-6">
          <Link href="/" className="text-3xl font-extrabold text-white hover:text-green-400 transition-colors duration-200">
            YOUR LOGO
          </Link>
        </div>
        ...
        <nav className="mb-6">
          <ul className="flex flex-wrap justify-center gap-x-6 gap-y-2 text-lg">
            {footerNavItems.map((item) => (
              <li key={item.name}>
                <Link href={item.href} className="hover:text-green-400 transition-colors duration-200">
                  {item.name}
                </Link>
              </li>
            ))}
